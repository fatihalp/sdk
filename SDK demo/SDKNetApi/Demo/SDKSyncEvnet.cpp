...
bool sync = false;
static void NotifySyncEvent(const HEventCore *core, void *userData) {
    // 不要在这里执行 SyncEvent, 这里是调度线程触发的回调. 执行是不生效的
    printf("syncEvent\n");
    fflush(stdout);
    
    // 换个方式通知创建事件核心的线程可以进行同步了
    // 如条件锁, 或者使用变量等方法, 这里采用变量方式简单演示
    sync = true;
}
...
int main()
{
    ...
    // 我想自己手动推动事件核心, 绑定同步事件通知回调
    BindSyncEventNotifyCallBack(core, NotifySyncEvent, nullptr);
    ...
    FreeNetSession(net);
    // 关闭手动同步核心事件
    FreeSyncEventNotifyCallBack(core);
    // 释放事件核心
    FreeEventCore(core);
    return 0;
}
